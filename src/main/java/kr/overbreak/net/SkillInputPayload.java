 ...
 * clientTime 클라이언트 시계로 본 그 순간 (1/20초 단위, 서브틱 포함) — 누른 채 있던 시간을 핑과 무관하게 잽니다 (0.2h, 참철 모아 베기)
 *
 * 스킬 발동 자체는 기존 경로(좌클릭 · 사용 · 웅크리기 · 손 바꾸기 · 버리기)가 하고, 이 패킷은 "언제" 만 알려 줍니다.
 * 서버는 값을 믿지 않고 범위로 자르고 너무 잦으면 버립니다 ({@link InputTiming}).
 */
public record SkillInputPayload(int slot, boolean pressed, float subTick, double clientTime) implements CustomPacketPayload {
	...
	public static final StreamCodec<ByteBuf, SkillInputPayload> CODEC = StreamCodec.composite(
			...
			ByteBufCodecs.DOUBLE, SkillInputPayload::clientTime,
			SkillInputPayload::new);

	/** 클라이언트 시계 없이 (시험 · 옛 경로). */
	public SkillInputPayload(int slot, boolean pressed, float subTick) {
		this(slot, pressed, subTick, Double.NaN);
	}
