...
public final class ScatterAnim {
	...
	private static final Fov[] FOV = {
			...
			new Fov(at(DASH_START, DASH, 0.4F), 1.40F, Ease.EXPO_OUT),
			new Fov(at(BRAKE_START, BRAKE, 0.0F), 1.40F, Ease.HOLD),
			// 난사 동안 넓은 시야를 유지 — 사방으로 쏘는 궤적이 화면에 다 들어오게
			new Fov(at(BRAKE_START, BRAKE, 0.7F), 1.32F, Ease.QUAD_OUT),
			new Fov(at(RECOVER_START, RECOVER, 0.0F), 1.32F, Ease.HOLD),
