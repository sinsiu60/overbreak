...
final class PatchNotes {
	...
	static final List<Version> VERSIONS = List.of(new Version("0.2i", "", "참철 강화", List.of(
			new Section("참철", 0xFFC8CDD6, List.of(
					...
					tweak("상대가 검막을 쓰면 그 순간 금빛으로 번쩍이고, 막는 동안 앞쪽에 금빛 방벽이 보입니다 (상대 화면에만)"),
					tweak("어깨 박치기 돌진에 바람 소리 · 시야각이 넓어지는 연출 · 1인칭 속도선이 생겼습니다"),
			...
			new Section("새 직업 — 참철 (대검 근접 결투형)", 0xFFC8CDD6, List.of(
					...
					tweak("방어 파쇄 — 2단 이상 모아 베기는 대상의 피해 감소 효과를 절반만 받게 합니다 (파워 블록 · 검막 등)"))),
			new Section("참철 — LMB 참철검", 0xFFC8CDD6, List.of(
					tweak("짧게 누르면 3타 콤보 — 오른 가로 베기 35 → 왼 가로 베기 35 → 내려찍기 55 · 후딜 중 누르면 이어지고 1.2초 쉬면 1타로"),
					tweak("0.25초 넘게 누르고 있으면 참 모으기 — 0.6초 40 · 1.2초 70 · 1.8초 110 (앞 60°)"),
					buff("1.8~2.0초 사이에 떼면 「진 참」 130 · 6칸 — 조준점 아래 게이지의 흰 칸"),
					tweak("모으는 중 이동 ×0.7 · 점프 불가 · 2.8초까지 누르고 있으면 저절로 3단 발동"),
					tweak("모으는 단계마다 대검이 초록 · 노랑 · 빨강으로 빛나 누구나 알아볼 수 있고, 진 참 창에서는 흰빛으로 번쩍입니다"),
					tweak("역경직 — 평타 · 모아 베기가 적을 맞히면 내 1인칭 화면의 칼이 순간 멈췄다가 빠르게 따라잡고 화면이 짧게 흔들립니다"))),
			new Section("참철 — 스킬", 0xFFC8CDD6, List.of(
					tweak("RMB 어깨 박치기 — 6칸 몸통 돌진, 처음 부딪친 적 20 + 2칸 밀어냄 · 돌진 중 받는 피해 ×0.5 · 모으는 중에도 쓸 수 있고 그동안 모으기 시간은 멈춤 · 7초"),
					tweak("SHIFT 검막 — 1초 동안 앞 120°에서 온 피해 ×0.2 · 막는 중 LMB 로 곧바로 모으기 · 막아 내면 1.5초 안에 시작한 모으기가 2단부터 · 9초"),
					tweak("E 대지 가르기 — 지면을 따라 8칸 달리는 칼날 (1칸 턱은 넘고 벽 · 절벽에서 사라짐) · 관통 25 + 1.5초 둔화 40% · 공중에서는 못 씀 · 10초"),
					tweak("Q 천참 — 1.2초 모았다가 (움직일 수 없음 · 넉백 · 군중제어 면역) 앞 14칸 × 폭 4칸을 벽째로 베어 140 · 베일 자리가 바닥에 붉게 표시됩니다"))))),
			new Version("0.2g", "", "커스텀 조준점", List.of(
			new Section("커스텀 조준점", 0xFF3FA2FF, List.of(
					tweak("오버워치처럼 조준점을 직접 꾸밀 수 있습니다 — F9 또는 로비의 「조준점」"),
					tweak("모양 (십자 · 원 · 원 + 십자 · 점 · 마인크래프트 기본) · 색 (8가지 + 빨강 · 초록 · 파랑 직접) · 두께 · 길이 · 가운데 간격 · 원 반지름"),
					tweak("불투명도 · 검은 테두리 · 가운데 점 크기 · 점 불투명도 — 하늘 · 풀밭 · 어두운 곳 미리보기로 바로 확인, 닫으면 저장"))),
			new Section("궤적의 깃털", 0xFF7FD4FF, List.of(
					buff("사선 앵커를 점프로 끊어도 가던 관성이 그대로 남습니다 — 올라가다 끊으면 속도가 뚝 끊기던 것 수정"),
					buff("벽 끝에 걸고 점프로 끊으면 다시 벽을 넘어갈 수 있습니다"),
					tweak("대신 끊은 뒤에는 갈고리 자리보다 1.5칸 위까지만 솟구칩니다 — 수직으로 걸고 끊어도 하늘 높이 솟지 않음"))))),
			new Version("0.2f", "", "궤적 추격", List.of(
			new Section("전 직업", 0xFF3FA2FF, List.of(
					buff("공중에서도 이동키로 방향을 틀고 가속할 수 있습니다 — 시선이 아니라 이동키 기준이라 적을 겨눈 채 좌우로 돕니다"),
					tweak("손을 떼면 관성으로 서서히 느려집니다 · 넉백을 맞은 뒤 0.3초와 기절 · 에어본 · 스킬 추진 중에는 제어되지 않습니다"),
					tweak("마인크래프트 기본 배경 음악이 나오지 않습니다 (경기 막판 음악은 그대로)"))),
			new Section("궤적의 깃털 — 새 궁극기 「궤적 추격」 (Q)", 0xFF7FD4FF, List.of(
					tweak("궤적 해방이 「궤적 추격」 으로 바뀌었습니다"),
					tweak("6초 동안 자유 비행 — 이동키로 초당 10칸 · 점프 키로 상승 · 떼면 천천히 가라앉음 · 땅에 닿아도 끝나지 않음"),
					buff("비행 중 평타는 조준 12° 안의 적에게 휘어 날아가는 유도 탄 — 비행 중이라 모두 치명타 30 · 탄창 무한"),
					tweak("유도 탄은 벽을 돌아 휘지 못하고, 조준점 둘레 원 안에 잡힌 적에게 표식이 뜹니다"),
					tweak("궁극기 동안 등 뒤에 하늘색 빛 날개 · 발밑 빛 고리 · 머리 위 빛기둥이 떠서 누구나 한눈에 알아봅니다"),
					tweak("비행 중에도 반동 도약 · 사선 앵커 · 돌진 난사를 모두 쓸 수 있습니다"),
					nerf("기절 · 에어본을 맞으면 곧바로 끝나고 떨어집니다 (군중제어 면역 없음) · 궁극기 중에는 게이지가 차지 않습니다"))),
			new Section("궤적의 깃털", 0xFF7FD4FF, List.of(
					buff("체공 훈풍: 공중 제어가 다른 직업보다 강합니다 — 약 0.25초 만에 초당 6칸"),
					tweak("반동 도약이 조준한 방향으로 8칸 날아가고, 충격탄은 출발 지점에서 터져 근처 적을 바깥으로 밀어냅니다"),
					tweak("반동 도약 — 수평을 봐도 약 1.5칸 떠오르고, 바닥(50° 아래)을 보면 곧장 위로 약 7칸 · 적은 뚫고 지나가고 벽에 막히면 멈춤"),
					buff("돌진 난사가 벽에 부딪혀도 끊기지 않고 난사까지 그대로 갑니다"),
					buff("사선 앵커를 벽 끝 가까이에 걸면 몸이 갈고리 위로 지나가 벽을 넘어갑니다"),
					nerf("사선 앵커를 점프로 끊을 때 위로 솟구치는 속도를 줄였습니다 — 수직으로 걸고 끊으면 너무 높이 올라갔습니다"))))),
			new Version("0.2e", "", "궤적 해방", List.of(
			new Section("전 직업", 0xFF3FA2FF, List.of(
					buff("재장전 중에 스킬을 쓰면 재장전을 끊고 스킬이 바로 나갑니다 — 궤적의 깃털 · 보안관 · 발키리 (탄창은 그대로)"),
					tweak("3인칭에서 1인칭으로 바꿀 때 0.3초 동안 카메라가 뒤에서 부드럽게 당겨 들어옵니다"))),
			new Section("궤적의 깃털 — 새 궁극기 「궤적 해방」 (Q)", 0xFF7FD4FF, List.of(
					tweak("차원 회전 포격이 「궤적 해방」 으로 바뀌었습니다"),
					tweak("발동 즉시 5초 동안 탄창 무한 · 재장전 없음 — 쏜 총알마다 그 궤적이 하늘색 선으로 공중에 남습니다 (최대 25줄)"),
					tweak("궤적은 총구에서 맞은 곳 / 벽 / 16칸 중 가장 가까운 곳까지 · 공중에서 쏜 궤적은 굵고 밝은 치명 궤적"),
					tweak("5초가 지나거나 1초 뒤부터 Q 를 다시 누르면 0.5초 예고(번쩍임 · 차징음) 뒤 모든 궤적이 동시에 폭발"),
					tweak("폭발 — 궤적 1칸 안의 적에게 줄당 12 (치명 궤적 18), 여러 줄은 누적 · 적 1명당 최대 120 · 넉백 없음"),
					buff("궁극기 동안 반동 도약 쿨타임 1.5초 · 평타 피해와 이동기 쿨타임 환급은 평소 그대로"),
					nerf("쓰는 사람이 죽으면 궤적은 폭발 없이 흘러내리며 사라집니다 · 적에게도 궤적이 모두 보입니다"),
					nerf("궁극기 동안에는 게이지가 차지 않습니다 · 예고 0.5초 동안은 쏠 수 없습니다"))),
			new Section("궤적의 깃털", 0xFF7FD4FF, List.of(
					buff("돌진 난사 한 번당 피해 7 → 12 (전부 맞으면 42 → 72)"),
					buff("돌진 난사로 적을 한 명 맞힐 때마다 쌍권총에 2발 장전"),
					tweak("돌진 난사 소리를 전용 녹음으로 바꿨습니다 — 발사음이 분신 총구 자리에서 사방으로 들리고, 맞히면 나에게만 적중음"),
					tweak("돌진 난사 중에는 F5 를 눌러도 1인칭으로 바뀌지 않습니다"),
					buff("공중 치명타 높이 2칸 → 1.5칸"),
					buff("사선 앵커 사거리 12칸 → 16칸"),
					buff("사선 앵커로 끌려가는 중 점프 키를 누르면 와이어를 끊고 그 속도 그대로 날아갑니다"))))),
			new Version("0.2d", "", "분신 난사", List.of(
			new Section("궤적의 깃털 — 쌍권총", 0xFF7FD4FF, List.of(
					tweak("총알 궤적이 하늘색으로 바뀌었습니다"),
					tweak("오른손 · 왼손이 번갈아 쏘고, 쏜 총의 총구가 반동으로 들립니다 (1인칭 · 3인칭)"),
					tweak("1인칭 총알 궤적이 화면 가운데가 아니라 쏜 총의 총구에서 나갑니다"),
					buff("공중에서 쏠 때 뒤로 밀려나던 반동이 없어졌습니다"))),
			new Section("궤적의 깃털 — 돌진 난사 (웅크리기)", 0xFF7FD4FF, List.of(
					tweak("곡예 난사(웅크리기 + RMB)가 「돌진 난사」(웅크리기)로 바뀌었습니다 — 바라보는 방향으로 6칸 돌진한 뒤 사방을 쓸어 버립니다"),
					tweak("돌진은 위아래까지 바라보는 방향 그대로 나갑니다 · 땅에서 아래를 보고 쓰면 정면으로 · 적은 뚫고 지나가고 벽에 막히면 멈춥니다"),
					nerf("난사 — 0.15초마다 6번, 반경 5칸 안 모든 적에게 7 (최대 42) · 벽 너머는 맞지 않음 · 넉백 없음"),
					nerf("난사 중 이동 속도 절반 · 무적 없음 · 균열 지대 위에서는 쓸 수 없음"),
					buff("쿨타임 9초 → 8초"),
					buff("공중에서 쓰면 난사가 끝날 때까지 그 자리에 떠 있습니다"),
					tweak("기절 · 에어본으로 끊기면 그 자리에서 끝납니다"),
					tweak("난사가 시작되면 본체 대신 분신이 8칸 안을 휘저으며 적을 쏩니다 — 내 모습의 분신이 자리를 옮기며 서서히 투명해지고, 멈춘 자리마다 하늘색 분신이 자세를 바꿔 가며 쏩니다"),
					tweak("쓰는 동안 3인칭으로 바뀌고 시야각이 넓어지며 발밑에 판정 원이 보입니다"))),
			new Section("궤적의 깃털 — 체공 훈풍", 0xFF7FD4FF, List.of(
					tweak("활공이 「공중에서 웅크리기」 에서 「점프 키 누르고 있기」 로 바뀌었고, 떨어지기 시작한 뒤부터 켜집니다"),
					buff("반동 도약 · 사선 앵커를 쓸 때마다 활공 시간 +1초 (착지하면 2초로)"),
					nerf("공중 치명타는 땅에서 2칸 이상 떠 있을 때만 붙습니다"),
					tweak("재장전 · 돌진 난사 중에는 활공 동작이 그 동작을 덮지 않습니다 (느린 낙하는 그대로)"))),
			new Section("근접 1인칭 평타", 0xFFFF7A4A, List.of(
					tweak("워리어 철도끼 — 오른쪽 위로 치켜들었다가 왼쪽 아래로 내려베기"),
					tweak("햄머나이트 철퇴 — 머리 위로 들었다가 곧장 내리찍고 반동으로 한 번 튐"),
					tweak("셰이드 그림자 검 — 짧게 뺐다가 화면을 가로로 훑는 빠른 베기"),
					tweak("정방향 · 역방향이 서로 반대 궤적으로 번갈아 나옵니다"))),
			new Section("조작 공통", 0xFF3FA2FF, List.of(
					tweak("웅크리기 키를 눌러도 몸이 주저앉지 않고, 난간 끝에서 멈춰 서지도 않습니다 — 웅크리기는 스킬 키입니다"),
					tweak("이동 속도가 바뀌어도 시야각(FOV)이 변하지 않습니다 (스킬 연출의 줌은 그대로)"),
					tweak("F5 시점 전환에서 정면 3인칭이 빠졌습니다 — 1인칭과 어깨 너머 3인칭 둘만 돕니다"))),
			new Section("매치 음악", 0xFFFFC23A, List.of(
					tweak("1대1 · 팀 매치가 매치 포인트에 들어서면 배경 음악이 낮게 깔립니다"),
					tweak("대난투는 1등이 목표까지 10점 남았을 때부터 나옵니다"),
					tweak("음악 볼륨 설정을 따릅니다"))))),
			new Version("0.2c", "", "새 직업 · 궤적의 깃털", List.of(
