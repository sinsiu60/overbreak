...
final class PatchNotes {
	...
	static final List<Version> VERSIONS = List.of(new Version("0.2d", "", "죽음의 꽃, 그리고 손맛", List.of(
			new Section("매치 음악", 0xFFFFC23A, List.of(
					tweak("1대1 · 팀 매치가 매치 포인트에 들어서면 배경 음악이 낮게 깔립니다"),
					tweak("대난투는 1등이 목표까지 10점 남았을 때부터 모두에게 나옵니다"),
					tweak("천천히 커졌다가 경기가 끝나면 천천히 사라집니다 · 음악 볼륨 설정을 따릅니다"))),
			new Section("조작 공통", 0xFF3FA2FF, List.of(
					tweak("웅크리기 키를 눌러도 이제 몸이 주저앉지 않습니다 — 웅크리기는 스킬 키라 눈높이가 꺼지면 조준이 흔들렸습니다"),
					tweak("웅크리기를 누르고 있어도 난간 끝에서 멈춰 서지 않습니다 — 웅크리기로 쓰는 이동기가 난간에서 끊겼습니다"),
					tweak("이동 속도가 느려지거나 빨라져도 시야각(FOV)이 바뀌지 않습니다 — 둔화 · 가속 · 달리기 모두 (스킬 연출의 줌은 그대로)"),
					tweak("F5 시점 전환에서 내 얼굴이 보이는 정면 3인칭이 빠졌습니다 — 1인칭과 어깨 너머 3인칭 둘만 돕니다"))),
			new Section("근접 1인칭 평타", 0xFFFF7A4A, List.of(
					tweak("워리어 · 햄머나이트 · 셰이드가 같은 「손 흔들기」 동작을 쓰던 것을 무기에 맞는 궤적으로 나눴습니다"),
					tweak("워리어 철도끼 — 오른쪽 위로 치켜들었다가 왼쪽 아래로 내려베기"),
					tweak("햄머나이트 철퇴 — 머리 위로 들었다가 곧장 내리찍고 반동으로 한 번 튐 (가장 무겁게)"),
					tweak("셰이드 그림자 검 — 짧게 뺐다가 화면을 가로로 훑는 빠른 베기 (가장 가볍게)"),
					tweak("정방향 · 역방향이 서로 반대 궤적으로 번갈아 나옵니다"))),
			new Section("궤적의 깃털 — 쌍권총", 0xFF7FD4FF, List.of(
					tweak("총알 궤적이 하늘색으로 바뀌었습니다"),
					tweak("첫 발은 오른손, 다음 발은 왼손 — 총구도 반동도 좌우로 번갈아 나옵니다 (1인칭 · 3인칭 모두)"),
					buff("공중에서 쏠 때 뒤로 밀려나던 반동을 없앴습니다 — 조준선이 흔들려 평타가 제자리에 꽂히지 않았습니다"),
					tweak("뒤로 날아가는 것은 반동 도약(RMB)이 맡습니다"),
					tweak("쏠 때 그 총의 총구가 위로 들립니다 — 오른손 · 왼손 각각 따로"),
					tweak("1인칭 총알 궤적이 화면 가운데 허공이 아니라 쏜 총의 총구에서 나갑니다"))),
			new Section("궤적의 깃털 — 돌진 난사 (웅크리기 · 새 스킬)", 0xFF7FD4FF, List.of(
					tweak("곡예 난사가 「돌진 난사」 로 바뀌었습니다 — 앞으로 치고 나가 멈춰 선 뒤 사방을 쓸어 버립니다"),
					tweak("기 모으기 0.1초 → 돌진 0.25초 → 제동 0.1초 → 난사 0.9초 → 마무리 0.25초 (모두 1.6초)"),
					tweak("돌진 — 바라보는 방향 그대로(위아래 포함) 정확히 6칸 · 땅 위에서 아래를 보고 쓰면 정면으로 나갑니다"),
					tweak("공중에서 멈춰 쏘면 난사가 끝날 때까지 떨어지지 않고 그 자리에 떠 있습니다 (마무리부터 다시 떨어짐)"),
					tweak("돌진은 적을 뚫고 지나가고, 벽에 막히면 곧장 제동으로 넘어갑니다"),
					nerf("난사 — 0.15초마다 6번 · 반경 5칸 안 모든 적에게 한 번당 7 (전부 맞으면 42) · 벽 너머는 맞지 않습니다"),
					nerf("난사 중에는 이동 속도가 절반이 됩니다 · 넉백 없음"),
					nerf("도는 동안의 무적이 없어졌습니다 · 이동기라 균열 지대 위에서는 쓸 수 없습니다"),
					buff("쿨타임 9초 → 8초 (쓰는 즉시)"),
					tweak("기절 · 에어본으로 끊기면 그 자리에서 끝납니다 (난사 전이면 난사는 나가지 않음)"),
					tweak("쓰는 동안 3인칭 · 발밑 판정 원 · 잔상 · 제동과 한 발마다 화면 흔들림 · 시야각 펀치"),
					tweak("3인칭 몸 동작 — 앞으로 숙여 돌진, 미끄러지며 양팔을 활짝, 한 바퀴 반을 돌고 반대로 휘청이며, 발을 구르고, 마무리에 총을 한 바퀴 돌려 잡습니다"),
					tweak("두 팔이 제각각 바깥 · 하늘 · 땅 · 몸 너머로 마구잡이로 총을 휘두르고, 총알 궤적 · 탄피 · 발사음이 그 총구 방향 그대로 나갑니다 (판정은 그대로 반경 5칸 전체)"),
					tweak("같은 난사는 모든 사람 화면에서 같은 방향으로 보입니다 · 늦게 본 사람도 지금 동작부터 이어서 봅니다"),
					tweak("누르는 즉시 동작이 시작됩니다 (서버 확인을 기다리지 않음)"),
					tweak("난사 발사음을 폭발음에서 가벼운 권총 소리로 바꾸고 볼륨을 낮췄습니다"),
					tweak("쓰는 동안 시야각이 크게 넓어져 난사 내내 유지됩니다 (돌진 1.4배 → 난사 1.32배 → 마무리에서 원래대로)"),
					tweak("난사가 시작되면 본체가 사라지고 분신이 8칸 안을 휘젓습니다 — 내 모습의 분신이 쉬지 않고 자리를 옮기며 서서히 투명해지고, 멈춘 자리마다 하늘색 분신이 남아 곁의 적을 쏩니다"),
					tweak("하늘색 분신은 한 발마다 무릎 쏴 · 엇갈려 쏘기 · 뒤돌아 쏘기 등 자세를 바꿔 가며 쏩니다 (판정은 그대로 본체 둘레 5칸)"),
					tweak("사방으로 나가는 총알 궤적이 하늘색으로 더 굵고 밝아졌고 0.25초 동안 남습니다 · 총구 화염도 하늘색"),
					tweak("잔상이 스킨 색과 상관없이 맑은 하늘색입니다 (셰이드는 그대로 짙은 파랑)"),
					tweak("잔상 빈도를 절반으로 줄였습니다 — 너무 촘촘해 몸이 뭉개져 보였습니다"))),
			new Section("궤적의 깃털 — 체공 훈풍", 0xFF7FD4FF, List.of(
					tweak("활공이 「공중에서 웅크리기」 에서 「점프 키 누르고 있기」 로 옮겼습니다 — 웅크리기가 스킬 키가 되었기 때문입니다"),
					nerf("활공은 떨어지기 시작한 뒤부터만 켜집니다 — 그냥 점프만 해도 활공이 켜지던 것 수정"),
					buff("반동 도약 · 사선 앵커를 쓸 때마다 활공 시간 +1초 (착지하면 2초로)"),
					nerf("공중 치명타는 땅에서 2칸 이상 떠 있을 때만 — 제자리 점프로는 붙지 않습니다"),
					tweak("재장전 중에는 활공 동작이 뜨지 않습니다 — 공중 재장전 동작을 덮어써 끊어먹었습니다 (느린 낙하는 그대로)"),
					tweak("돌진 난사 중에도 활공 동작이 뜨지 않습니다 — 난사 몸 동작을 덮어썼습니다 (느린 낙하는 그대로)"))))),
