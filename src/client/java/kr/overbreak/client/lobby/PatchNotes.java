...
final class PatchNotes {
	...
	static final List<Version> VERSIONS = List.of(new Version("0.2c", "", "새 직업 · 궤적의 깃털", List.of(
			new Section("새 직업 — 궤적의 깃털 (공중 기동 원거리형)", 0xFF7FD4FF, List.of(
					buff("체력 180 · 낙하 피해를 아예 받지 않는 새 직업이 들어왔습니다"),
					tweak("쌍권총 연사 (LMB) — 16칸 히트스캔 · 발당 20 · 0.2초에 한 발 · 탄창 18발"),
					buff("체공 훈풍 (패시브) — 공중에서 맞힌 총알은 무조건 치명타 150%"),
					buff("공중에서 맞힐 때마다 반동 도약 · 사선 앵커의 쿨타임이 0.5초씩 깎입니다"),
					tweak("공중에서 웅크리면 2초 활공 — 낙하 속도 -80% · 착지하면 다시 찹니다"),
					tweak("반동 도약 (RMB) — 조준한 곳에 반경 3칸 25, 그 반동으로 정반대로 약 7칸. 쿨타임 6초"),
					tweak("곡예 난사 (웅크리기 + RMB) — 한 바퀴 돌며 사방으로 8발 · 발당 15 · 도는 0.8초 무적. 쿨타임 9초"),
					tweak("사선 앵커 (E) — 12칸 와이어 · 적중 20 + 0.5초 기절 + 그 적의 머리 위로 · 벽이면 그 자리로. 쿨타임 7초"),
					tweak("차원 회전 포격 (Q) — 약 6칸 솟구쳐 멈춰 선 채 3초간 반경 5칸에 초당 60 (최대 180)"),
					tweak("포격 중에는 반동 도약을 쿨타임 없이 써서 자리를 옮길 수 있습니다"))),
			new Section("공중 재장전", 0xFF7FD4FF, List.of(
					tweak("탄창 두 개를 위로 튕겨 올리고 두 총을 한 바퀴 돌린 뒤 받아 끼웁니다 (1.25초)"),
					tweak("맞물리는 순간 화면이 살짝 숙였다 들리고 시야각이 짧게 조였다 돌아옵니다"),
					nerf("재장전 1.25초 동안에는 사격도 스킬도 나가지 않습니다"))),
			new Section("스킬 UI", 0xFF9AA0AC, List.of(
					tweak("E 스킬이 쿨타임일 때 E 를 누르면 칸이 빨간색으로 흔들립니다 — 소리만 나던 것 수정 (F 에만 붙어 있었습니다)"))))),
			new Version("0.2a", "", "손맛과 시야", List.of(
